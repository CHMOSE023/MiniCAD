...
namespace MiniCAD {
    ...
    public:
        ...
        // --- 正交投影参数（全部 Real） ---
        void SetOrthoExtent(Real left, Real right, Real bottom, Real top, Real nearZ, Real farZ);
        void SetOrthoFromViewport(Real viewportWidth, Real viewportHeight,  Real zoom, Real nearZ, Real farZ);
        ...
        Ray    ScreenToRay(const Vec2& screenPos, Real viewportWidth, Real viewportHeight) const;
        Point3 ScreenToWorld(const Vec2& screenPos, Real depth, Real viewportWidth, Real viewportHeight) const;
        Vec2   WorldToScreen(const Point3& worldPos, Real viewportWidth, Real viewportHeight) const;

        // Picker 注入回调
        using ScreenToRayFn = std::function<Ray(const Vec2&)>;
        ScreenToRayFn MakeScreenToRayCallback(Real viewportWidth,  Real viewportHeight) const;

        Real  GetOrthoWidth() { return m_orthoRight - m_orthoLeft; };
        Real  GetOrthoHeight() { return m_orthoTop - m_orthoBottom; }; 
    private:
