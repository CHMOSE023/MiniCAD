...
namespace MiniCAD {
    ...
    bool D3D11Renderer::Initialize(void* hwnd, int width, int height)
    {
        ...
        hr = m_impl->swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&backBuffer));
    ...
    void D3D11Renderer::Resize(int width, int height)
    {
        ...
        HRESULT hr = m_impl->swapChain->ResizeBuffers(0, static_cast<UINT>(width), static_cast<UINT>(height), DXGI_FORMAT_UNKNOWN, 0);
        ...
        if (SUCCEEDED(hr)) 
        {
            ...
            if (SUCCEEDED(hr)) 
            {
                m_impl->device->CreateRenderTargetView( backBuffer, nullptr, &m_impl->renderTargetView);
